...
$$\mcalI=\sum_{k\geq 0}\mcalI_{k}'x^k$$
其中$\mcalI_{k}'\in\bbR\fps{\hbar}$为形式幂级数。
记$\mcalI_k:=\mcalI_{k}'x^k$，
于是$\mcalI$被分解为$\mcalI=\sum_{k=0}^\infty\mcalI_k$.
...
（2）$W$为集合，$\fai:V\to W$
\end{definition}
我们称$W$为权集，在这里$W=\{\mcalI_0,\mcalI_1,...\}\subseteq\bbR[x]\fps{\hbar}^+$.

我们可以按照加权的类型对顶点带权的带标记的有向图分类。
比如，此图有$d_1$个顶点赋以权$\mcalI_{k_1}$，
...
=\sum_{G\in\bbZ\atop D\geq 0}
   \lmd_{G,D}\hbar^Gx^D
$$
其中$\lmd_{G,D}\in\bbR$.为证明良定性，我们需要说明以下三点：
